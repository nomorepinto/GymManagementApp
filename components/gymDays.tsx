...
export default function GymDays({ gymDays, setGymDays }: { gymDays: string[], setGymDays: any }) {
    ...
    return (
        <View className="flex justify-start bg-primarybg rounded-3xl mb-5 p-5">
            <View className="max-h-60 mb-2">
                <ScrollView className="flex-grow-0">
                    {gymDays.length > 0 ? (
                        gymDays.map((day, index) => (
                            <View key={index} className="bg-secondarybg rounded-3xl p-2 mb-2 flex flex-row justify-between items-center">

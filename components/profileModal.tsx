    ...
    return (
        <Modal animationType="slide" transparent={true} visible={isOpen}>
            <View className="flex-1 items-center justify-center">
                <View className="w-3/4 flex rounded-3xl p-5 bg-secondarybg">
                    <View className="flex justify-end self-center">
                        <Text className="text-white text-2xl font-nexaHeavy">Select Profile</Text>
                    </View>
                    <View className="max-h-40 px-5 rounded-3xl bg-primarybg mt-5">
                        <ScrollView className="flex-grow-0 w-full">
                            {profileArray.map((profile) => (
                                <Pressable key={profile.id} className="flex p-2 border-b border-slate-600 active:bg-slate-700 rounded-md" onPress={() => handleSelectProfile(profile)}>
                                    <Text className="text-white text-2xl font-nexaLight">{profile.name} Profile</Text>
                                </Pressable>
                            ))}
                            <Pressable key="createNewProfile" className="bg-primarytext flex px-2 m-2 border-b border-slate-600 active:bg-slate-700 rounded-full" onPress={handleCreateNewProfile}>
